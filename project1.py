...
class User:
    ...
    def deposit(self,amount):
        if amount > 0:
            ...
            print(f'Succesfully Deposit!!!"\n"Deposit amount is {amount} and Current balance {self.balance}')
            self.transaction_history.append("Deposited is : " + str(amount) + ' by ' + str(self.name) + " date : " + str(date.today()))
    ...
    def withdraw(self,amount):
        if amount <= self.balance:
            ...
            self.transaction_history.append("Withdraw is : " + str(amount) + " by " + str(self.name)+ " date : " + str(date.today()))
            if self.id in self.store_users:
                use = self.store_users[self.id] 
                use['balance'] = self.balance
        else:
    ...
    def show_available_balance(self):
        print(f"{self.name} available balance is {self.store_users[self.id]['balance']}")

    def transfer_balance_another_account(self,account_no,amount):
        if account_no in self.store_users:
            if amount <= self.balance:
                self.transaction_history.append("transfer account id is "+ str(account_no) + " money -> " + str(amount)+ " date : " + str(date.today()))
    ...
    def take_loan_from_bank(self,amount):
        ...
        if amount < twice_amount:
            if self.bank.given_loan(amount):
                ...
                self.transaction_history.append("take Loan from bank : " + str(amount)+ " date : " + str(date.today()))
                if self.id in self.store_users:
                    use = self.store_users[self.id]
                    use['balance'] = self.balance
            else:
...
class Admin:
    ...
    def show_total_balance(self):
        print('The total avialable balance is : ',self.bank.total_balance, " of bank")

    def show_total_loan_amount(self):
        print('The total loan amount is : ', self.bank.total_loan_amount, 'of bank')
